...
function getCacheUri(ctx: ExtensionContext) {
  return Uri.joinPath(ctx.globalStorageUri, 'i18n-set-cache')
}
...
async function writeCache(ctx: ExtensionContext, iconSetId: string, data: any) {
  try {
...
async function loadCache(ctx: ExtensionContext, iconSetId: string): Promise<any | undefined> {
  try {
...
async function migrateCache(ctx: ExtensionContext) {
  ...
  for (const key of ctx.globalState.keys()) {
    if (key.startsWith(prefix)) {
      const cached = ctx.globalState.get<any>(key)!
